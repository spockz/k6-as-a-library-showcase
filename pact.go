...
type preparedPactRequest struct {
...
func bindPactInteractionURLs(interactions []pactInteraction, base *url.URL) error {
	if base == nil {
		return errors.New("PACT target URL is nil")
	}
	for index := range interactions {
		resolved, err := resolvePactURL(base, interactions[index].Request.Path, interactions[index].Request.Query)
		if err != nil {
			return fmt.Errorf("resolve PACT interaction %q URL: %w", interactions[index].Name, err)
		}
		requestURL, err := httpext.NewURL(resolved.String(), interactions[index].Name)
		if err != nil {
			return fmt.Errorf("create PACT interaction %q URL: %w", interactions[index].Name, err)
		}
		interactions[index].RequestURL = &requestURL
	}
	return nil
}

func resolvePactURL(base *url.URL, pactPath string, rawQuery json.RawMessage) (*url.URL, error) {
	if base == nil {
		return nil, errors.New("PACT target URL is nil")
	}
	pathURL, err := url.Parse(pactPath)
	if err != nil {
		return nil, fmt.Errorf("parse request path %q: %w", pactPath, err)
	}
	if pathURL.IsAbs() || pathURL.Host != "" || pathURL.Fragment != "" {
		return nil, fmt.Errorf("request path %q must be a relative path", pactPath)
	}

	resolved := *base
	resolved.Path = joinPactPath(base.Path, pathURL.Path)
	resolved.RawPath = ""
	resolved.RawQuery = pathURL.RawQuery
	if len(bytes.TrimSpace(rawQuery)) != 0 && !bytes.Equal(bytes.TrimSpace(rawQuery), []byte("null")) {
		query, err := pactQueryString(rawQuery)
		if err != nil {
			return nil, err
		}
...
func (interaction pactInteraction) prepareRequest() (preparedPactRequest, error) {
	if interaction.RequestURL == nil || interaction.RequestURL.GetURL() == nil {
		return preparedPactRequest{}, errors.New("PACT interaction URL is not bound")
	}
	requestURL := *interaction.RequestURL.GetURL()
	request := &http.Request{
		Method: strings.ToUpper(interaction.Request.Method),
		URL:    &requestURL,
		Header: make(http.Header),
	}
	if err := applyPactHeaders(request, interaction.Request.Headers); err != nil {
		return preparedPactRequest{}, err
	}

	var body *bytes.Buffer
	if interaction.Request.Body != nil {
		bodyBytes, err := pactBodyBytes(interaction.Request.Body)
		if err != nil {
			return preparedPactRequest{}, fmt.Errorf("encode request body: %w", err)
		}
		body = bytes.NewBuffer(bodyBytes)
	}
	cookies, err := pactRequestCookies(interaction.Request.Cookies)
	if err != nil {
		return preparedPactRequest{}, fmt.Errorf("decode request cookies: %w", err)
	}

	return preparedPactRequest{
		Request: request,
		Body:    body,
		Cookies: cookies,
	}, nil
...
func pactRequestCookies(rawCookies json.RawMessage) (map[string]*httpext.HTTPRequestCookie, error) {
	cookies := make(map[string]*httpext.HTTPRequestCookie)
	trimmed := bytes.TrimSpace(rawCookies)
	...
	for name, rawValue := range rawValues {
		values, err := pactRawStringValues(rawValue)
		if err != nil {
			return nil, fmt.Errorf("cookie %q: %w", name, err)
		}
		if len(values) == 0 {
			continue
		}
		cookies[name] = &httpext.HTTPRequestCookie{Name: name, Value: values[0], Replace: true}
	}
	return cookies, nil
}
...
func applyPactHeaders(request *http.Request, rawHeaders map[string]json.RawMessage) error {
	...
	for _, key := range keys {
		...
		if err != nil {
			return fmt.Errorf("decode request header %q: %w", key, err)
		}
...
func matchPactResponse(expected pactHTTPResponse, actual *httpext.Response) error {
	if actual == nil {
		return errors.New("response was not received")
	}

	var mismatches []error
	if actual.Status != expected.Status {
		mismatches = append(mismatches, fmt.Errorf("status: expected %d, got %d", expected.Status, actual.Status))
	}
	...
	return fmt.Errorf("PACT response mismatch: %w", errors.Join(mismatches...))
}
