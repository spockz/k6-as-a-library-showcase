...
func resolveRunConfig(config runConfig, environment map[string]string) (runConfig, error) {
	if !config.outputsFlagSet {
		if raw, ok := environment["K6_OUT"]; ok {
			outputs, err := benchmarkpkg.ParseOutputSelection(raw)
			if err != nil {
	...
	config.outputs = benchmarkpkg.NormalizeOutputNames(config.outputs)
	...
	if !config.tracesOutputFlagSet {
		if value, ok := environment["K6_TRACES_OUTPUT"]; ok {
			...
			config.tracesOutput = benchmarkpkg.DefaultTraceOutput
		}
	}
	traceConfiguration, err := benchmarkpkg.ParseTraceConfiguration(config.tracesOutput)
	if err != nil {
