...
func TestRunCommandRejectsInvalidArguments(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "agreement rejects VUs", args: []string{"run", "--agreements", "agreements.yaml", "--vus", "2"}},
		{name: "agreement rejects iterations", args: []string{"run", "--agreements", "agreements.yaml", "--iterations", "2"}},
		{name: "agreement rejects max duration", args: []string{"run", "--agreements", "agreements.yaml", "--max-duration", "1m"}},
		{name: "scaling requires agreement", args: []string{"run", "--load-scaling-factor", "2"}},
		{name: "explicit one scaling requires agreement", args: []string{"run", "--load-scaling-factor", "1"}},
		{name: "invalid agreement scaling", args: []string{"run", "--agreements", "agreements.yaml", "--load-scaling-factor", "invalid"}},
		{name: "zero planned operations", args: []string{"run", "--agreements", "agreements.yaml", "--max-planned-operations", "0"}},
		{name: "zero generator VUs", args: []string{"run", "--agreements", "agreements.yaml", "--generator-max-vus", "0"}},
